#!/usr/bin/env julia
"""
Uncertainty Set Analysis — demand bounds + raw order data.
...
Usage (from project root):
    ...
    julia scripts/analysis/analyze_uncertainty_set.jl <bounds_file> <order_file> <out_dir>
"""
...
bounds_file   = length(ARGS) >= 1 ? abspath(ARGS[1]) : joinpath(PROJECT_ROOT, "Data", "demand_bounds.json")
order_file    = length(ARGS) >= 2 ? abspath(ARGS[2]) : joinpath(PROJECT_ROOT, "Data", "zhuzhou_data", "order.csv")
out_dir       = length(ARGS) >= 3 ? abspath(ARGS[3]) : joinpath(PROJECT_ROOT, "experiments", "demand_understanding")
mkpath(out_dir)
