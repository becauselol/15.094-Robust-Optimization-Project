...
Microtransit is a form of transit that seeks to fill the gap between the existing services such as private-hire vehicles and scheduled public transit. Microtransit is an on-demand, publicly shared-ride service. It is like private-hire, in the sense that it is flexible and on-demand, but it also benefits from the economies of scale of scheduled public transportation by using mid-size capacity vehicles and allowing for a higher vehicle load.

However, having a fully flexible microtransit system is overly complex and reduces its aggregation capabilities, often having to make significant detours just to satisfy one or two customers. Thus, selection of these \emph{virtual bus stops} (VBS) becomes an important strategic decision since the optimal selection will allow for better aggregation, decreased waiting time and less waiting time for passengers, resulting in an overall improvement of customer experience as well as reduction in service costs.

While the VBS selection is able to adapt between service periods within the day (Morning, Afternoon, Evening), the selection for each service period should be robust against shifts in the demand patterns. Within a service period, there can be a fluctuation on a day to day basis and the system should be robust and still provide a relatively similar experience across each day. We thus address this problem with a two-stage robust model where origin-destination (OD) demand varies within a budget uncertainty set.
...
\paragraph{Walking admissibility.}
For each OD pair $(o,d)\in\Omega$, define the admissible pickup--dropoff station
pair set
\[
	A_{od}
	=
	...
	\right\}.
\]
The assignment variable $x_{odjks}$ is defined only for $(j,k)\in A_{od}$.
...
We calibrate on April 1--3, 2025 and evaluate on May 1--31, 2025 --- an
intentionally data-poor split that mimics operators who have not yet observed
dense historical demand. We fix $l=30$ built stations, a 300 s walking limit,
$\lambda=10$, and vary $k \in \{10,15,20\}$. For the robust model we vary the
calibration quantile $q \in \{0.90,0.95,0.99\}$.
...
both 40-station variants produce large box-to-cap ratios, with the sparse
instance starting from much thinner OD coverage per period.
...
pairs. We simulate this by filtering calibration to the top 10\% of OD pairs
...
ensuring that peripheral demand is not left uncovered --- rather than cost
